...
class MolGraph:
    def __init__(self, init_state=None, sanitize_mol=False, mutability=True, sulfur_valence=6):
        ...
        bonds.
        :param mutability: value of the mutability flag to all atoms of the initial state
        """
    ...
    def get_atom_mutability(self, id):
        ...
        :return:
        """
        mutabil = self.mol_graph.GetAtomWithIdx(id).GetBoolProp("mutability")
        return mutabil
    ...
    def copy(self):
        ...
        new_qumol_graph.mol_graph = RWMol(self.mol_graph, True)#quickCopy=true, the resulting ROMol will not copy any of the properties or bookmarks and conformers from other.
        new_qumol_graph.update_mol_representation() #Sanitize the copied mol (explicit cycles, bonds, (no aromatic), hydrogen , …)
    ...
    def add_bond(self, from_at, to_at, update_repr=True):
        """
        Increasing by 1, the bond between the two atoms of given ids according to the following scheme.
        None -> Simple
        Simple -> Double
        Double -> Triple
        If bonding is impossible because of valence, an exception is returned.
        :param from_at: bond left atom index
        :param to_at: bond right atom index 
        :return:
        """
        # Extracting current bond between given atoms
    ...
    def get_bridge_bonds_matrix(self):
        ...
        molecular graph are bridges. Bond is considered as a bridge if its deletion lead to upgrade of connex compound number.
        """
    ...
    def get_atom_degree(self, id, as_multigraph):
        """
        Returning the degree of the atom of given id.
        ...
        @param id: index of the mol to consider
        @param as_multigraph : whether the index of atom as multigraph or not
        :return:
        """
        #If as_multigraph is set to True, the degree of the atom considers the types of the bonds and is equivalent to the explicit valence.
        if as_multigraph:
            return self._get_expl_valence(id)
        else: #If as_multigraph is set to False, the degree of the atom is defined as the number of atoms it shares bonds with.  
    ...
    def get_formal_charge(self, at_idx):
        ...
        """
        formal_ch = self.mol_graph.GetAtomWithIdx(at_idx).GetFormalCharge()
        return formal_ch
    ...
    def get_max_valence(self, atom_type):
        ...
        if atom_type == "S":
            ...
            val = GetPeriodicTable().GetDefaultValence(GetPeriodicTable().GetAtomicNumber(atom_type))
            return val
    ...
    def _get_expl_valence(self, at_idx):
        ...
        explicitVal = at.GetExplicitValence()
        return explicitVal
    ...
    def draw(self, at_idx=False, show=True, size=200, write_to_path=None):
        ...
        opts = dr.drawOptions().addAtomIndices = True  # Edit atoms indexes
    ...
    def get_n_atoms(self):
        NumAtom = self.mol_graph.GetNumAtoms()
        return NumAtom
