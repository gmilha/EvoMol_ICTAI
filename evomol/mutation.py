...
from evomol.evaluation import EvaluationError, RDFiltersEvaluationStrategy, SillyWalksEvaluationStrategy, \
    SAScoreEvaluationStrategy
from evomol.molgraphops.molgraph import MolGraphBuilder
from evomol.molgraphops.exploration import random_neighbour, RandomActionTypeSelectionStrategy, DeterministQLearningActionSelectionStrategy

from rdkit.Chem import MolFromSmiles
from evomol.molgraphops.molgraph import MolGraph
from evomol.notification import Observable
...
class KRandomGraphOpsImprovingMutationStrategy(MutationStrategy):
    ...
    def mutate(self, individual, ind_to_replace_idx, curr_total_score, pop_tabu_list, external_tabu_list,
               generated_ind_recorder):
        # Drawing the number of actions (depth)
        ...
        for i in range(self.max_n_try):
            ...
            if not failed_tabu_pop and not failed_tabu_external and not failed_quality_filter and \
                ...
                if self.evaluation_strategy.keys()[0] == 'silly_walks':
                    curr_total_score = 0 # In order is_improved method is correct even for 1st steps if no threshold mentioned    
...
class QLearningGraphOpsImprovingMutationStrategy(MutationStrategy, Observable):
    """
    Performing a graph operations mutation composed of at most k actions. The mutation is performed at most n_max_try
    to find an improver. If no improver is found, raising a MutationError.
    """
    ...
    def __init__(self, k, max_n_try, evaluation_strategy, action_spaces, action_spaces_parameters,
        ...
        :param k: max number of successive graph operations (depth)
        :param max_n_try: max number of tries to find an improver
        :param evaluation_strategy: EvaluationStrategy instance with an evaluate_individual method
        :param action_spaces: list of ActionSpace instances
        :param action_spaces_parameters: instance of ActionSpace.ActionSpaceParameters
        ...
        self.neighbour_gen_strategy = neighbour_gen_strategy if neighbour_gen_strategy is not None else \
            RandomActionTypeSelectionStrategy()
        ...
        # Adding the neighbour generation strategy to the list of observers
        self.subscribe(self.neighbour_gen_strategy)
    ...
    def mutate(self, individual, ind_to_replace_idx, curr_total_score, pop_tabu_list, external_tabu_list,
               generated_ind_recorder):
        #Management of reward, by default not inverted, inverted if objective function is a min problem type
        inverted_reward = False 
        if self.problem_type == "min":
            inverted_reward = True 
        
        # Management of the boolean reward
        BoolReward = False
        if self.evaluation_strategy.keys()[0] == 'silly_walks':
            BoolReward = True
            curr_total_score = 0 # In order is_improved method is correct even for 1st steps
        ...
        for i in range(self.max_n_try):
            ...
            try:
                ...
                # Performing mutation
                mutated_ind, desc, new_qumol_builders, executed_actions, executed_actions_ecfp_Context = self.neighbour_gen_strategy.generate_neighbour(qumol_builder.copy(),
                                                                                       n_actions, evaluation_strategy=self.evaluation_strategy,
                                                                                       return_mol_graph=True)
            except Exception as e:
                print(e)
                raise MutationError(individual.to_aromatic_smiles()) from e
            ...
            if not failed_tabu_pop and not failed_tabu_external and not failed_quality_filter and \
                ...
                for (mol_builder, action) in zip([qumol_builder] + new_qumol_builders[0:-1], executed_actions):
                    self.notify_observers(mol_builder, action, executed_actions_ecfp_Context, mutated_total_score, inverted_reward, BoolReward)
                ...
                # Returning the mutated solution if it is an improver
                if is_improver:
                    print("Improver - Returning the mutated solution")
                    return mutated_ind, desc, mutated_total_score, mutated_scores, evaluation_time

            # The mutated solution did not pass the tabu filters
            elif failed_tabu_pop or failed_tabu_external:
                print("The mutated solution did not pass the tabu filters")
                generated_ind_recorder.record_individual(individual=mutated_ind,
                                                         total_score=None,
                                                         scores=np.full((len(self.evaluation_strategy.keys(), )), None),
                                                         objective_calls=self.evaluation_strategy.n_calls,
            ...
            # The mutated solution did not pass the filters (tabu lists excluded)
                ...
                print("The mutated solution did not pass the filters (tabu lists excluded)")
                generated_ind_recorder.record_individual(individual=mutated_ind,
                                                         total_score=None,
                                                         scores=np.full((len(self.evaluation_strategy.keys(), )), None),
                                                         objective_calls=self.evaluation_strategy.n_calls,
                ...
                # Updating Q Learning weights
                for (mol_builder, action) in zip([qumol_builder] + new_qumol_builders[0:-1], executed_actions):
                    self.notify_observers(mol_builder, action, executed_actions_ecfp_Context, 1., inverted_reward, BoolReward)
