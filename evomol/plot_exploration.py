...
def extract_history_data(filepath):
    ...
    with open(filepath, "r", newline='') as f:#r: file open in read only mode; newline='' ppermits to manage proberly csv files import 
        reader = csv.reader(f)
        keys = []
        for index, row in enumerate(reader): #for i, row in enumerate(reader):
            if index == 0: #if index == 0:
                for key in row:
                    keys.append(key) #keys contains the list of variables names
            else:
                scores_dict = {}
...
def exploration_graph(model_path, neighbours_threshold=0, root_node="C", plot_images=False,
    ...
    if plot_images:
        img_labels = draw_mol_labels(labels, actions_history_smi_pop, actions_history_smi_removed, actions_history_scores_pop, actions_history_scores_removed, 
