...
def _parse_neighbour_generation_parameters(explicit_search_parameters, evaluation_strategy, action_spaces, action_spaces_parameters,
    ...
    if explicit_search_parameters["neighbour_generation_strategy"] == RandomActionTypeSelectionStrategy:
        ...
        neighbour_generation_strategy = explicit_search_parameters["neighbour_generation_strategy"](
            ...
            alpha=explicit_QL_parameters["alpha"],
            epsilon=explicit_QL_parameters["epsilon"],
            epsilon_min=explicit_QL_parameters["epsilon_min"],
            epsilon_0=explicit_QL_parameters["epsilon_0"],
            lambd=explicit_QL_parameters["lambd"],
            epsilon_method=explicit_QL_parameters["epsilon_method"]
        )
    else:
...
def _extract_explicit_search_parameters(parameters_dict):
    ...
    explicit_search_parameters = {
        ...
        "pop_max_size": input_search_parameters["pop_max_size"] if "pop_max_size" in input_search_parameters else 2000,
...
def _extract_explicit_QL_parameters(parameters_dict):
    ...
    explicit_QL_parameters = {
        "alpha": input_QL_parameters["alpha"] if "alpha" in input_QL_parameters else 0.4,
        "epsilon": input_QL_parameters["epsilon"] if "epsilon" in input_QL_parameters else 0.2,
        "epsilon_min": input_QL_parameters["epsilon_min"] if "epsilon_min" in input_QL_parameters else 0.2,
        "epsilon_0": input_QL_parameters["epsilon_0"] if "epsilon_0" in input_QL_parameters else 1,
        "lambd": input_QL_parameters["lambd"] if "lambd" in input_QL_parameters else 0.01,
        "epsilon_method": input_QL_parameters["epsilon_method"] if "epsilon_method" in input_QL_parameters else "power_law",
