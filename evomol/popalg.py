...
class GeneratedIndividualsRecorder:
    ...
    def record_individual(self, individual, total_score, scores, objective_calls, improver, success_obj_computation,
        ...
        #self.TOTAL_objective_calls.append(TOTAL_objective_calls)
...
class PopAlg:
    ...
    def copy_instance_with_parameters(self):
        """
        Copying the instance with (only) the parameters. 
        The strategies must be set for each copied instance.
        Allows parallelism.
        """
    ...
    def __init__(self, evaluation_strategy, mutation_strategy, stop_criterion_strategy,
        ...
        :param record_history: Whether to record history of actions (necessary to draw exploration trees)(default :False)
        ...
        :param kth_score_to_record_key: string key of the kth best score to be recorded dynamically (default : "total")
        :param shuffle_init_pop: whether to shuffle initial population at initialization (default :False)
        :param external_tabu_list: list of SMILES that cannot be generated by EvoMol (default :None)
        ...
        that failed the objective computation (default :False)
        :param evaluation_strategy_parameters: allows to set evaluation_strategy parameters depending on context.
        Available contexts are "evaluate_new_solution" and "evaluate_init_pop" (default :None)
        :param sulfur_valence: maximum valence of sulfur atoms (default : 6)
        :param record_trained_weights: whether to record the weights of the trained model (default :False)
        """
    ...
    def initialize(self):
        ...
        self.step_traces = {
            ...
            'n_objective_calls': [],      # Added
        
            'n_not_improvers': [],
            "TOTAL_not_improvers": [],

            'n_discarded_tabu': [],
            'TOTAL_discarded_tabu': [],
            'n_discarded_filters': [],
            'n_discarded_rdfilters': [],
            'n_discarded_sillywalks': [],
            'TOTAL_discarded_sillywalks': [],
...
# Convertor for JSON files
    def convert_np(self, obj):
        ...
        if isinstance(obj, np.ndarray):
            return obj.tolist()
        raise TypeError(f"Type {type(obj)} non supporté")
    
    def save(self):
        """
        Saving generated data into files
        :return:
        """
...
#            csv_array.append(["n_objective_calls"] + self.step_traces["n_objective_calls"])
            ...
            csv_array.append(["n_not_improvers"] +
                             self.step_traces["n_not_improvers"])
            csv_array.append(["TOTAL_not_improvers"] + self.step_traces["TOTAL_not_improvers"])
            csv_array.append(["n_discarded_tabu"] + self.step_traces["n_discarded_tabu"])
            csv_array.append(["TOTAL_discarded_tabu"] + self.step_traces["TOTAL_discarded_tabu"])
            ...
            csv_array.append(["TOTAL_discarded_sillywalks"] + self.step_traces["TOTAL_discarded_sillywalks"])
            #csv_array.append(["PCT_discarded_sillywalks"] + self.step_traces["PCT_discarded_sillywalks"])
            ...
            # ### Saving steps data from csv_array to steps.csv file###
            with open(join(self.output_folder_path, 'steps.csv'), "w") as f:
                writer = csv.writer(f)
                for row in np.array(csv_array).T:
                    writer.writerow(row)

            # ### Save all generated individuals data ###
            if self.record_all_generated_individuals:
                # ### Updating csv_array with generated_individuals data###
                ...
                # ### Saving all_generated_individuals data from csv_array to all_generated.csv file###
                with open(join(self.output_folder_path, "all_generated.csv"), "w") as f:
                    writer = csv.writer(f)
                    for row in np.array(csv_array).T:
                        writer.writerow(row)
            
            # ### Saving final population history and data ###
            csv_array = []
            csv_array.append(["smiles"] + self.pop_tabu_list)

            # Mutation success and failures history
            n_success_mut_str = []
            n_fail_mut_str = []
            ...
            # ### Removed individuals actions recording if required ###
            ...
            # ### Trained weights data if required  ###
            if self.record_trained_weights:
                #### if DeterministQLearningActionSelectionStrategy used
                if isinstance(self.mutation_strategy.neighbour_gen_strategy, DeterministQLearningActionSelectionStrategy):
                    ...
                    if self.curr_step_id == 0:
                        with open(join(self.output_folder_path, 'addA_weights.csv'), "w", newline='') as f:
                            ...
                            writer.writerow([str(self.curr_step_id)] + list(w_addA))
                        with open(join(self.output_folder_path, 'rmA_weights.csv'), "w", newline='') as f:
                            ...
                            writer.writerow([str(self.curr_step_id)] + list(w_rmA))
                        with open(join(self.output_folder_path, 'chB_weights.csv'), "w", newline='') as f:
                            ...
                            writer.writerow([str(self.curr_step_id)] + list(w_chB))                            
                    else:
                ...
                #### if StochasticQLearningActionSelectionStrategy used
                elif isinstance(self.mutation_strategy.neighbour_gen_strategy, StochasticQLearningActionSelectionStrategy):
                    for action in ['AddA', 'RmA', 'ChB']:
                        #Save decod of ecfp# link to the action
                        Decod_action = self.mutation_strategy.neighbour_gen_strategy.AllFeatDecod[action]
                        with open(join(self.output_folder_path,action+"_Decod.json"), "w") as f:
                                json.dump(Decod_action, f, default = self.convert_np, indent=4)
                        w_action = dict(self.mutation_strategy.neighbour_gen_strategy.Weights[action])        
                        for i, wType in enumerate(["use", "success", "success_rates"]) :
                            #success_rate calculated from use and success value to manage properly not used contexts
                            Wcurr_step = {key:w_action[key][i] for idx, key in enumerate(w_action)}
                            
                            file_name = join(self.output_folder_path, action+"_"+wType)
                            # Loading file_name.json if exists
                            try:
                                with open(join(self.output_folder_path,file_name+".json"), "r") as f:
                                    Wsteps = json.load(f)  
                            except FileNotFoundError:
                                Wsteps = [] #if not existing file (step0; create empty df)
                                                        
                            Wsteps.append(Wcurr_step)
                              
                            with open(file_name+".json", "w") as f:
                                json.dump(Wsteps, f, indent=4)
                                
                            # save as csv file with ecfp idx in sorted order
                            all_positions = sorted(set(int(pos) for Wstep in Wsteps for pos in Wstep.keys()))
                            
                            df = pd.DataFrame([{pos: Wstep.get(pos, 0) for pos in all_positions} for Wstep in Wsteps])
                            df.index.name = "Step"
    ...
    def record_step_data(self, step_gen_ind_recorder):
        ...
        # Saving current step scores statistics 
        for k, scores_list in step_scores_dict.items():
        ...
        # Keep running totals
        total_not_improvers = sum(self.step_traces["n_not_improvers"]) 
        total_discarded_tabu = sum(self.step_traces["n_discarded_tabu"])
        total_discarded_sillywalks = sum(self.step_traces["n_discarded_sillywalks"])

        # Saving information about discarded solutions during the step
        if step_gen_ind_recorder is not None:
            # Get previous and current objective calls to calculate the number of calls during the step
            if len(self.step_traces['additional_values']['TOTAL_objective_calls']) == 1:
                step_n_calls = 0
            else:
                previous_n_calls = self.step_traces['additional_values']['TOTAL_objective_calls'][len(self.step_traces['additional_values']['TOTAL_objective_calls'])-2]
                curr_n_calls = self.step_traces['additional_values']['TOTAL_objective_calls'][len(self.step_traces['additional_values']['TOTAL_objective_calls'])-1]
                step_n_calls = curr_n_calls - previous_n_calls
            
            self.step_traces['n_objective_calls'].append(step_n_calls)
            ...
            self.step_traces["n_not_improvers"].append(np.sum(
                np.logical_not(step_gen_ind_recorder.improver)[np.logical_and(
                    np.array(step_gen_ind_recorder.success_obj_computation),
                    step_gen_ind_recorder.get_passed_filters_mask())]
            ))
            self.step_traces["TOTAL_not_improvers"].append(total_not_improvers+np.sum(
                np.logical_not(step_gen_ind_recorder.improver)[np.logical_and(
                    np.array(step_gen_ind_recorder.success_obj_computation),
                    step_gen_ind_recorder.get_passed_filters_mask())]
            ))
            ...
            self.step_traces["TOTAL_discarded_tabu"].append(total_discarded_tabu+np.sum(np.logical_or(
                step_gen_ind_recorder.failed_tabu_pop, step_gen_ind_recorder.failed_tabu_external)))
            ...
            self.step_traces["TOTAL_discarded_sillywalks"].append(total_discarded_sillywalks+np.sum(step_gen_ind_recorder.failed_sillywalks))
            if step_n_calls == 0:
                pct_silly = 0
            else:
                pct_silly = np.sum(step_gen_ind_recorder.failed_sillywalks) / step_n_calls
            self.step_traces["PCT_discarded_sillywalks"].append(pct_silly)
            ...
            self.step_traces["n_not_improvers"].append(0)
            self.step_traces["TOTAL_not_improvers"].append(0)
            self.step_traces["n_discarded_tabu"].append(0)
            self.step_traces["TOTAL_discarded_tabu"].append(0)
            ...
            self.step_traces["TOTAL_discarded_sillywalks"].append(0)
    ...
    def sort_to_be_mutated(self):
        ...
        # Individuals selection depending the selection scenario defined
    ...
    def get_k_best_individuals_smiles(self, k_best, tabu_list=None):
        ...
        """
        # Ascending sort of curr_total_scores
        scores_sort = np.argsort(self.curr_total_scores)

        #Revert sort if problem type is max
        if self.problem_type == "max":
            scores_sort = scores_sort[::-1]

        # List individuals, total score and transposed sub scores 
                ...
                try:
                    ...
                    for curr_to_be_replaced_idx in curr_to_be_replaced_indices:
                        ...
                        while not replacement_successful:
                            # Select individual to mutate if available
                            if n_mutated_tries < len(to_be_mutated_indices):
                                ...
                                try:
                                    ...
                                    if curr_to_be_replaced_idx < len(self.curr_total_scores):
                                        curr_to_be_replaced_total_score = self.curr_total_scores[curr_to_be_replaced_idx]
                                    ...
                                    # #### Draw all_generated mol graph ###
                                    graph = MolGraph(MolFromSmiles(mutated_ind.to_aromatic_smiles()))
                                    
                                    # Define mol ecfp and bits activate in the new generated mol
                                    mol = MolFromSmiles(mutated_ind.to_smiles())
                                    Molfingerprint = AllChem.GetMorganFingerprintAsBitVect(mol, radius=2, nBits=1024)
                                    active_bits = list(Molfingerprint.GetOnBits())
